...
# Normalize for career completeness — recent classes have fewer years tracked
ltv_df['LTV_PER_PLAYER_YR']   = (ltv_df['TOTAL_PERF'] / (ltv_df['N_PLAYERS'] * ltv_df['AVG_CAREER_YRS'])).round(3)
# Flag classes where most players haven't reached career year 8 yet
ltv_df['CAREER_COMPLETE']     = ltv_df['DRAFT_YEAR'] <= 2014   # 2014 class has 9+ years available
...
# Use normalized LTV (per player per career year) for fair cross-class comparison
...
# Right: normalized LTV trend — only complete-career classes
...
for bucket in PICK_ORDER:
    ...
    active10 = sub[sub['CAREER_YEAR']>=10]['PLAYER_ID'].nunique()
    reach_yr10[bucket] = round(active10/total*100, 1)
...
# ── 11. Key Findings ─────────────────────────────────────────────────────────
# Use normalized metric; restrict best/worst to career-complete classes
complete   = ltv_df[ltv_df['CAREER_COMPLETE']]
