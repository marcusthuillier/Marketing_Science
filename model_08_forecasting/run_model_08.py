...
def run_forecasts(team_season_df):
    print("\nRunning baseline + ARIMA forecasts for each team-season...")
    results = []
    ...
    for i, ((season, team_id), g) in enumerate(groups):
        ...
        wins_at_half = g.loc[g["GAME_NUM"] == HALF, "CUM_WINS"].iloc[0]
...
def find_notable_examples(results_df):
    ...
    return best_for_arima, worst_for_arima


# ── Visualizations ───────────────────────────────────────────────────────────

def generate_plots(team_season_df, results_df, best_for_arima, worst_for_arima, summary, show=False):
    matplotlib.use("Agg")
    plt.rcParams.update({"font.family": "DejaVu Sans", "font.size": 10})

    # ── Plot 1: trajectory chart for illustrative teams ─────────────────────
    # Pick the team-season where ARIMA helped most and where it hurt most --
    # these are the most instructive trajectories to show.
    illustrative = pd.concat([best_for_arima.head(1), worst_for_arima.head(1)])
    ...
    fig.suptitle("Mid-Season Win Pace vs Actual Finish -- Baseline vs ARIMA",
    ...
    width = 0.32
    bars1 = ax.bar(x - width/2, baseline_vals, width, color=BLUE, alpha=0.85, label="Baseline (pace extrapolation)")
    bars2 = ax.bar(x + width/2, arima_vals, width, color=RED, alpha=0.85, label="ARIMA")
    ...
    ax.set_title(
        f"Baseline vs ARIMA Forecast Error\n"
        f"n={summary['n_team_seasons']} team-seasons  |  ARIMA beats baseline in {summary['pct_arima_better']:.1%} of cases",
...
def print_findings(summary, best_for_arima, worst_for_arima):
    print(f"\n{'='*60}")
    print("  KEY FINDINGS -- NBA Standings Forecasting")
    print(f"{'='*60}")
    print(f"  Team-seasons analyzed: {summary['n_team_seasons']}")
    print(f"  Baseline RMSE:         {summary['rmse_baseline']:.3f} wins")
    print(f"  ARIMA RMSE:            {summary['rmse_arima']:.3f} wins")
    print(f"  Baseline MAE:          {summary['mae_baseline']:.3f} wins")
    print(f"  ARIMA MAE:             {summary['mae_arima']:.3f} wins")
    print(f"  ARIMA beats baseline:  {summary['pct_arima_better']:.1%} of team-seasons")

    winner = "ARIMA" if summary["rmse_arima"] < summary["rmse_baseline"] else "Baseline"
...
if __name__ == "__main__":
    ...
    best_for_arima, worst_for_arima = find_notable_examples(results_df)
    generate_plots(team_season_df, results_df, best_for_arima, worst_for_arima, summary, show=False)
    print_findings(summary, best_for_arima, worst_for_arima)
